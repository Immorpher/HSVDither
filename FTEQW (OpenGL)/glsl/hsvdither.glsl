...
vec4 colround(vec2 position, vec4 color){ // Rounding function
	...
	c *= vec3(r_hsvd_clevels+1,r_hsvd_clevels,r_hsvd_blevels); // color levels have +1 since both ends of hue are the same
	...
	c /= vec3(r_hsvd_clevels+1,r_hsvd_clevels,r_hsvd_blevels); // re-normalize back to 0 to 1
